...
export default function GoodsMySuppliesPage() {
  ...
  const loadRequests = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(API_ENDPOINTS.requests.mine('goods', 'supply'), fetchAuth);
        ...
        <div className="border-b border-violet-100 bg-gradient-to-bl from-violet-50/70 to-white px-4 py-5 sm:px-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-xl font-bold text-gray-900 sm:text-2xl">عرضه‌های من</h1>
              <p className="mt-2 max-w-xl text-sm leading-relaxed text-gray-600">
                عرضه‌های کالایی که ثبت کرده‌اید و پیشنهادهای خریداران را اینجا پیگیری کنید.
              </p>
            </div>
            <Link href="/goods/supplies/new" className={`${primaryBtnClass} shrink-0 bg-violet-600 hover:bg-violet-700`}>
              ...
              {FILTERS.map((item) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
                    filter === item.id ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                >
                  {item.label}
                </button>
      ...
      {filteredRequests.length > 0 ? (
        ...
        <div className={`${cardClass} px-4 py-12 text-center sm:px-6`}>
          ...
          <h2 className="mt-4 text-lg font-bold text-gray-900">
            {requests.length === 0 ? 'هنوز عرضه‌ای ثبت نکرده‌اید' : 'موردی با این فیلتر یافت نشد'}
          </h2>
          {requests.length === 0 ? (
            <Link href="/goods/supplies/new" className={`${primaryBtnClass} mt-5 inline-flex bg-violet-600 hover:bg-violet-700`}>
              <PlusIcon className="h-4 w-4" aria-hidden />
              ثبت اولین عرضه
            </Link>
          ) : null}
...
function StatCard({ label, value, accent = false }) {
  return (
    <div className={`rounded-xl border px-3 py-3 ${accent ? 'border-violet-200 bg-violet-50/60' : 'border-gray-100 bg-gray-50/60'}`}>
